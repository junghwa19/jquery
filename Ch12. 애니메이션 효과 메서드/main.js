...
-css속성의 transition 속성과 비슷한 느낌이지만 애니메이션 메서드로 더 디테일하게 사용 할 수 있다.
...
$(document).ready(function(){
    ...
    //정의된 값을 사용하여 적용
    //width, height 같은 속성은 값에 show, hide, toggle 등 시각효과 메서드를 쓸 수 있따.
    $('.btn4').click(function(){
        $('.block6').animate({
            height: 'toggle'
        },'show');
    ...
    //Queue(대기열) 적용
    //큐는 먼저 쓴 코드 먼저 실행하고, 나중에 쓴 고드를 실행하는 것을 의미한다.
    $('.btn5').click(function(){
        $('.block7').animate({
            left: 300
        },2000);
        //시각효과 메서드나 animate() 메서드에 시간차가 있는 경우 뒤에 명령보다 먼저 실행되지만
        // 그시간을 다른 명령이 기다리지는 않는다.
    ...
    $('.btn6').click(function(){
        ...
        //첫 번째 매개변수가 true일 때 큐 대기중인 애니메이션 모두 제거
